...
static yolox_err_t detect_yolox(struct YoloX *conf, const cv::Mat &bgr, struct DetectResult *result) {
  ncnn::Net yolox;
  ...
  if (yolox.load_param(conf->param_path)) return YOLOX_ERROR;
  if (yolox.load_model(conf->model_path)) return YOLOX_ERROR;

  if (conf->nms_thresh <= 0) conf->nms_thresh = YOLOX_NMS_THRESH;
  if (conf->conf_thresh <= 0) conf->conf_thresh = YOLOX_CONF_THRESH;
  if (conf->target_size <= 0) conf->target_size = YOLOX_TARGET_SIZE;
  ...
  float scale = 1.f;
  if (w > h) {
    scale = conf->target_size / w;
    w = conf->target_size;
    h = h * scale;
  } else {
    scale = conf->target_size / h;
    h = conf->target_size;
    w = w * scale;
  }
...

  {
    ...
    generate_yolox_proposals(grid_strides, out, conf->conf_thresh, proposals);
  }
  ...
  nms_sorted_bboxes(proposals, picked, conf->nms_thresh);
...
struct DetectResult *detect_result_create() {
  return (DetectResult *) malloc(sizeof(struct DetectResult));
}
...
yolox_err_t detect(struct YoloX *yolox, const char *image_path, struct DetectResult *result) {
  ...
  return detect_yolox(yolox, m, result);
}
